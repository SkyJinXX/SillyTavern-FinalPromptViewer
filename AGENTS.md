...
## 协作注意：GitHub 安装的酒馆扩展需要 commit/push 后才会更新

本项目在 SillyTavern 里通常通过 GitHub 链接安装。修改本地项目文件后，酒馆不会自动读取本地工作区的未提交改动。

### 操作建议
- 每次完成修复后，先在本地验证。
- 然后 `git commit` 并 `git push` 到 GitHub。
- 再到 SillyTavern 的扩展管理里更新/刷新该扩展，浏览器页面才会拿到新版本资源。

如果只改了本地文件但没有 push，酒馆里安装的 GitHub 版扩展可能仍然使用旧 CSS/JS，导致调试时看到“代码明明改了，页面 computed style 还是旧规则”的现象。
